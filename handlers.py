...
def pipeline_handler(
    ...
    message = f'<b>Project:</b> {status} <a href="{project_web_url}">{project_full_path}</a>\n'
        ...
        if "message_id" in ctx[pipeline_id]:
            ...
            if status_changed:
                ...
                bot.bot.edit_message_text(
                    chat_id=chat["id"], message_id=message_id, text=message, parse_mode='HTML', reply_markup=reply_markup
                )
            else:
                logging.info(
